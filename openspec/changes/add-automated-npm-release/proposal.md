...
- Complete the **publishable package metadata** that OIDC and the registry require: a `repository.url` matching the GitHub repository exactly, a MIT `LICENSE` file, `homepage`, `bugs`, `keywords`, and `publishConfig.access: public`. The `version` field becomes the sentinel `0.0.0-development`; git tags and the registry are the source of truth.
